...
sagaModuleHelp<- function(module,algorithm=NULL) {
  ...
  if (!is.null(algorithm)){  
    info<- system2('C:/OSGeo4W64/apps/saga/saga_cmd.exe',paste(module,algorithm),stderr = TRUE)
  } else{
    info<-system2("C:/OSGeo4W64/apps/saga/saga_cmd.exe",paste(module),stderr = TRUE)
  }
  options(warn=0)
  return(info)
}